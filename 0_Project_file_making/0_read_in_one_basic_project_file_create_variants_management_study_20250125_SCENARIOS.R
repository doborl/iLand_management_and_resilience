rm(list=ls()) 
#Laura Dobor, 2025.01.07
# this produce project files using one basic file by modifying it by the line-number.
#going on managements + browsing +salvagings as combined cases
# and models and scenarios.
# it is also writing a .bat file which you can run afterwards and it will run it all.
# be careful to change the good line

dataroot<-"E:/iLandDist - new version 2.0 - 2025/"
basefile<-paste0(dataroot,"BASIC_FOR_WINDTESTING.xml")
...
for (ss in 1:2) {
  ...
  scen<-scens[ss]
  write( " ", (paste0(dataroot, "RUN_models_",scen,"_20250414.bat")), append=FALSE)
...
fs<-list.files(paste0(dataroot,"scripts/wind/"),paste0(scen,"_LD.txt"))
...
for (i in 1:length(fs)) {
  ...
  n[1,1]<-"<?xml version=\"1.0\" encoding=\"utf-8\"?>"   # to print "  need to write this\"
  #output:
 
  n[16,1]<-paste0("<out>",mgm,"_",windcase,"_",model,"_",scen,".sqlite</out> ")
  print( n[16,1])
  
  n[17,1]<-paste0(      "<climate>CZ_region_FORESEEv4_", model, "_",scen,".sqlite</climate>")
  #log:
 
  n[22,1]<-paste0("<logFile>log/",mgm,"_",windcase,"_",model,"_",scen,".txt</logFile> ")
  print( n[22,1])
  ...
  n[175,1]<-paste0("<timeEventsFile>wind/",  f, "</timeEventsFile>")
  print( n[175,1])
  ...
  n[454,1]<-paste0("      <remove>",salv,"</remove>")
  print( n[454,1])
  
  
  #write out:
outproj<-paste0(mgm,"_",windcase,"_",model,"_",scen,".xml")
out<-paste0(dataroot,outproj)
write.table(n,out, sep=" ", row.names = F,quote=F, col.names = F)

# this is for running iland for 100 years
write( paste0("ilandc.exe ",outproj, " 100"), (paste0(dataroot, "RUN_models_",scen,"_20250414.bat")),  append=TRUE)
