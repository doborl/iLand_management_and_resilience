...
write( " ", (paste0(dataroot, "RUN_refclim_20250414.bat")), append=FALSE)
...
fs<-list.files(paste0(dataroot,"scripts/wind/"),"refclim_LD.txt")
...
for (i in 1:length(fs)) {
  ...
  n[16,1]<-paste0("<out>",mgm,"_",windcase,"_refclim.sqlite</out> ")
  print( n[16,1])
  
  
  #log:
 
  n[22,1]<-paste0("<logFile>log/",mgm,"_",windcase,"_refclim.txt</logFile> ")
  print( n[22,1])
  ...
  #write out:
outproj<-paste0(mgm,"_",windcase,"_refclim.xml")
out<-paste0(dataroot,outproj)
write.table(n,out, sep=" ", row.names = F,quote=F, col.names = F)


write( paste0("ilandc.exe ",outproj, " 100"), (paste0(dataroot, "RUN_refclim_20250414.bat")),  append=TRUE)
