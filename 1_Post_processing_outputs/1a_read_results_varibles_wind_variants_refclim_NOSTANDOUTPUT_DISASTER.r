...
# 2025.04.18.
# Reads in sqlite files of iLand model results, select and postprocess variables we need
# Write out csv files by topics
# Not doing plotting
...
# File with the species codes, decidious or evergreen
...
# Take only those which are specific case
o<-grep( paste0(version,"_"), fs)
fs<-fs[o]

print("number of files:")
print(length(fs))

length(grep("refclim",fs))
length(grep("rcp45",fs))
length(grep("rcp85",fs))


# we have 5 winds, 5 managements, 4 models, 2 scenarios = 100 per scenario
# refclim has 5 winds, 5 managements =25
...
for (i in (1:length(fs)))  {
  ...
  mgm<-strsplit(f, "_")[[1]][2]
  windcase<-strsplit(f, "_")[[1]][3]
  part1<-  strsplit(f, "_")[[1]][4]
  part2<-  strsplit(f, "_")[[1]][5]
  model<-paste(part1,part2,sep='_')
  rcp<-strsplit(f, "_")[[1]][6]
...
# the actual date is added into the files names:
write.csv(MF.all, paste0(dataroot,Sys.Date(),"_multifunctionality_",version,".csv"), row.names = FALSE)
write.csv(annual.data.all, paste0(dataroot,Sys.Date(),"_annual_data_",version,".csv"), row.names = FALSE)
write.csv(removals.all, paste0(dataroot,Sys.Date(),"_removals_",version,".csv"), row.names = FALSE)
write.csv(damage.all, paste0(dataroot,Sys.Date(),"_damages_",version,".csv"), row.names = FALSE)
write.csv(recovery.all, paste0(dataroot,Sys.Date(),"_recovery_",version,".csv"), row.names = FALSE)
write.csv(lnd, paste0(dataroot,Sys.Date(),"_landscape_",version,".csv"), row.names = FALSE)
