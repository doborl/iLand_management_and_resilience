...
MF.mid30yavg<-MF.all %>% group_by(rcp,model,mgm, windcase) %>% filter(year>36&year<65) %>% summarise(across(where(is.numeric), ~mean(.x, na.rm = TRUE)))%>% select(-year)
...
MFs.long<-pivot_wider(MFs %>% select(-sallmin,-sallmax),values_from = sall,names_from = mgm)

colnames(MFs.long)<-c("rcp","ADAPT","BAU","BIOEC","CONSERV","UNMAN")

g40<-ggRadar(data=MFs.long,mapping = aes(colour = rcp), 
              rescale = FALSE, interactive = FALSE, use.label = TRUE, size = 2,alpha=0.1,
              legend.position = "bottom", scales="free") +theme_bw()+#ylim(0,4)+
  ggtitle("")+
